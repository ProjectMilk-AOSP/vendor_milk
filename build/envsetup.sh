function __print_milk_functions_help() {
cat <<EOF
...
- milkgerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
- milkrebase:   Rebase a Gerrit change and push it again.
- milkremote:   Add git remote for LineageOS Gerrit Review.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/milk-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.milk.device | grep -q "$MILK_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $MILK_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function milkremote()
{
    ...
    local MILK_USER=$(git config --get review.review.milkos.org.username)
    if [ -z "$MILK_USER" ]
    then
        git remote add milk ssh://review.milkos.org:29418/$PFX$PROJECT
    else
        git remote add milk ssh://$MILK_USER@review.milkos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'milk' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.mik.device | grep -q "$MILK_BUILD");
    then
...
function installrecovery()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.milk.device | grep -q "$MILK_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        milkremote
        git push milk HEAD:refs/heads/'$1'
    fi
    '
}
...
function milkgerrit() {
    ...
    local user=`git config --get review.review.milkos.org.username`
...
function milkrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      milkrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.milkos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.milk.device | grep -q "$MILK_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MILK_FIXUP_COMMON_OUT ]; then
