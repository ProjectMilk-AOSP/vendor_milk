...
func init() {
	android.RegisterModuleType("milk_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = milkExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = milkExpandVariables(ctx, dep_file)
	...
	cmd := milkExpandVariables(ctx, String(g.properties.Cmd))
